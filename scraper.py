...
SCRAPER_VERSION = "2026-07-16-pagination-v6-price-review"
...
def fill_template(products: list[Product]) -> None:
    ...
    for product in products:
        ...
        for variant in product.variants:
            ...
            # Temu requires a positive base price. Some discontinued/out-of-stock
            # AkvaSport products are published with €0.00. Do not invent a price and
            # do not abort the whole export: place those SKUs in a review file.
    ...
    if output_row == 5:
        raise RuntimeError(
            "No valid SKU rows could be written. Check akvasport_needs_review.csv."
        )
    ...
    if review_rows:
        logger.warning(
            "%d SKU row(s) were excluded from the Temu workbook and saved to %s",
            len(review_rows),
            REVIEW_CSV,
        )
